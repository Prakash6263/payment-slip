...
function ExperienceListPage() {
  ...
  const load = async () => {
    const { data, error } = await supabase
      ...
      .select("id, employee_name, designation, joining_date, last_working_date, tenure, updated_at")
  ...
  const onDelete = async (id: string) => {
    if (!confirm("Delete this experience certificate?")) return;
    const { error } = await supabase.from("experience_certificates").delete().eq("id", id);
    if (error) { toast.error(error.message); return; }
  ...
  const filtered = (rows ?? []).filter((r) =>
    !q || [r.employee_name, r.designation, r.tenure].join(" ").toLowerCase().includes(q.toLowerCase()),
  );
    ...
    <div className="min-h-screen bg-background">
      <div className="border-b border-border/60 bg-card/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium">Experience Certificates</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Link to="/experience/new" className="inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-semibold text-white" style={{ background: "var(--gradient-brand)" }}>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-5 flex items-center justify-between gap-3">
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by employee, designation, or tenure…"
            className="w-full max-w-md rounded-md border border-border bg-card px-3 py-2 text-sm outline-none focus:border-primary" />
          <span className="text-xs text-muted-foreground">{filtered.length} certificate{filtered.length === 1 ? "" : "s"}</span>
        </div>
        ...
        {rows === null ? (
          ...
          <div className="rounded-xl border border-dashed border-border p-10 text-center">
            <p className="text-sm text-muted-foreground">No saved experience certificates yet.</p>
            <Link to="/experience/new" className="mt-3 inline-block text-sm font-semibold text-primary">Create your first certificate →</Link>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50 text-left text-xs uppercase tracking-wider text-muted-foreground">
                <tr>
                  ...
                  <th className="px-4 py-3">Last Working</th>
                  <th className="px-4 py-3 text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-muted/30 cursor-pointer" onClick={() => navigate({ to: "/experience/$id", params: { id: r.id } })}>
                    ...
                    <td className="px-4 py-3">{r.joining_date || "—"}</td>
                    <td className="px-4 py-3">{r.last_working_date || "—"}</td>
                    <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        <Link to="/experience/$id" params={{ id: r.id }}
