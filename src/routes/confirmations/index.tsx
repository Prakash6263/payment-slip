...
function ConfirmationsListPage() {
  ...
  const load = async () => {
    const { data, error } = await supabase
      ...
      .select("id, employee_name, designation, letter_date, joining_date, updated_at")
  ...
  const onDelete = async (id: string) => {
    if (!confirm("Delete this confirmation letter?")) return;
    const { error } = await supabase.from("confirmation_letters").delete().eq("id", id);
    if (error) { toast.error(error.message); return; }
  ...
  const filtered = (rows ?? []).filter((r) =>
    !q || [r.employee_name, r.designation, r.letter_date].join(" ").toLowerCase().includes(q.toLowerCase()),
  );
      ...
      <div className="border-b border-border/60 bg-card/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium">Confirmation Letters</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Link to="/confirmations/new" className="inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-semibold text-white" style={{ background: "var(--gradient-brand)" }}>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        {rows === null ? (
          ...
          <div className="rounded-xl border border-dashed border-border p-10 text-center">
            <p className="text-sm text-muted-foreground">No saved confirmation letters yet.</p>
            <Link to="/confirmations/new" className="mt-3 inline-block text-sm font-semibold text-primary">Create your first letter →</Link>
          </div>
        ) : (
          <div className="overflow-hidden rounded-xl border border-border bg-card">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-muted/30 cursor-pointer" onClick={() => navigate({ to: "/confirmations/$id", params: { id: r.id } })}>
                    ...
                    <td className="px-4 py-3">{r.letter_date || "—"}</td>
                    <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        <Link to="/confirmations/$id" params={{ id: r.id }}
