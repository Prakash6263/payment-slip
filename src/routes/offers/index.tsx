...
function OffersListPage() {
  ...
  const load = async () => {
    const { data, error } = await supabase
      .from("offer_letters")
      .select("id, employee_name, designation, joining_date, letter_date, updated_at")
  ...
  const filtered = (rows ?? []).filter((r) =>
    !q || [r.employee_name, r.designation, r.joining_date].join(" ").toLowerCase().includes(q.toLowerCase()),
  );
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          ...
          <div className="flex items-center gap-2">
            <Link to="/" className="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-3 py-2 text-sm font-medium hover:bg-muted">
              <FileText className="h-4 w-4" /> Salary Slips
            </Link>
            <Link to="/offers/new" className="inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-semibold text-white" style={{ background: "var(--gradient-brand)" }}>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-5 flex items-center justify-between gap-3">
          <input value={q} onChange={(e) => setQ(e.target.value)}
            placeholder="Search by employee, designation, or date…"
        ...
        {rows === null ? (
          ...
          <div className="rounded-xl border border-dashed border-border p-10 text-center">
            <p className="text-sm text-muted-foreground">No saved offer letters yet.</p>
            <Link to="/offers/new" className="mt-3 inline-block text-sm font-semibold text-primary">Create your first letter →</Link>
          </div>
        ) : (
          <div className="overflow-hidden rounded-xl border border-border bg-card">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-muted/30 cursor-pointer" onClick={() => navigate({ to: "/offers/$id", params: { id: r.id } })}>
                    ...
                    <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        <Link to="/offers/$id" params={{ id: r.id }}
